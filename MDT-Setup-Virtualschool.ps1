...
If ($Help)
{
    ...
    If ($Begin -eq "y")
    {
        ...
        If ($ConvertESD -eq "y")
        {
            ...
            $LangCode = Read-Host -Prompt "Enter the language code of the Windows download (default: de-DE)"
            If ($LangCode -eq '')
            {
                $LangCode = "de-DE" ## The language of the Windows to download. Example: en-US
            }

            $Edition = Read-Host -Prompt "Enter the edition to download (default: Education)"
            If ($Edition -eq '')
            {
                $Edition = "Education" ## The edition to download
            }

            $KMSKey = Read-Host -Prompt "Enter the KMS key for the Windows download (default: NW6C2-QMPVW-D7KKK-3GKT6-VCFB2)"
            If ($KMSKey -eq '')
            {
                $KMSKey = "NW6C2-QMPVW-D7KKK-3GKT6-VCFB2" ## This key is the KMS Education key from the Microsoft website and is public
            }
        }

        ## Share names and paths
        $MdtBuildShare = Read-Host -Prompt "Enter the local path of the Build share (default: E:\MDT\BuildShare)"
        If ($MdtBuildShare -eq '')
        {
            $MdtBuildShare = "E:\MDT\BuildShare" ## Local path of the Build share
        }
        ...
        $MdtDepShare = Read-Host -Prompt "Enter the local path of the Deployment share (default: E:\MDT\DeployShare)"
        If ($MdtDepShare -eq '')
        {
            $MdtDepShare = "E:\MDT\DeployShare" ## Local path of the Deployment share
        }
        ...
        $TZName = Read-Host -Prompt "Enter the time zone name (default: W. Europe Standard Time)"
        If ($TZName -eq '')
        {
            $TZName = "W. Europe Standard Time"## The time zone for Windows
        }

        $KbLocaleCode = Read-Host -Prompt "Enter the keyboard locale code for Windows (default: 0407:00000407)"
        If ($KbLocaleCode -eq '')
        {
            $KbLocaleCode = "0407:00000407" ## The keyboard locale for Windows
        }

        $UILang = Read-Host -Prompt "Enter the locale for the Windows deployment (default: de-DE)"
        If ($UILang -eq '')
        {
            $UILang = "de-DE" ## The UI locale for Windows
            $UsrLocale = "de-DE" ## The user locale for Windows
            $KbLocaleName = "de-DE" ## The keyboard locale name for Windows
        }
        ...
        $DomainName = Read-Host -Prompt "Enter the domain of the user above (eg. schule.intern)"
        $OU = Read-Host -Prompt "Enter the full AD path for newly imaged PCs (eg. OU=PCs,DC=schule,DC=intern)"
        ...
        If ($ConvertESD -eq "y")
        {
            ...
            Write-Host -Object "        Windows key for download: $KMSKey"
        }
        ...
        If ($Ready -eq "y")
        {
            ...
            New-SmbShare -Name "$MdtBuildShareName" -Path "$MdtBuildShare" -FullAccess Administratoren
            ...
            If ($ConvertESD -eq "y")
            {
                ...
                Write-Host "Use this key to download your Windows iso: $KMSKey"
            ...
            New-SmbShare -Name "$MdtDepShareName" -Path "$MdtDepShare" -FullAccess Administratoren
